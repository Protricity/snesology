...
class SearchPaths implements IExecutable, IBuildable, IRoutable
{
	...
	function execute(IRequest $Request) {
		...
		$SearchQuery = PathEntry::query()
			->limit("{$row_count} OFFSET {$offset}");

		$SearchTable = new HTMLPDOQueryTable($SearchQuery);
		$SearchTable->addSearchColumn(PathTable::COLUMN_PATH, 'path');
		$SearchTable->addSearchColumn(PathTable::COLUMN_TITLE, 'path-title');
        $SearchTable->addSearchColumn(PathTable::COLUMN_STATUS, 'path-status');
        $SearchTable->addSearchColumn(PathTable::COLUMN_CREATED, 'path-created');
